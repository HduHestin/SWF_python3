...
class SWFRawTag(_dumb_repr):
    ...
    def parse(self,s):
        pos = s.tell() # get current file position
        self.header = s.readtag_header() # read tag header
        self.pos_content = s.tell()
        s.f.seek(pos) # 重置文件读取位置到标签头开始处
...
class SWFStraightEdge:
    ...
    def __repr__(self):
        # 定义类的字符串表示形式，这对应于Python 2中的_dumb_repr
        return '<SWFStraightEdge start={0} to={1} line_style_idx={2} fill_style_idx={3}>'.format(
            self.start, self.to, self.line_style_idx, self.fill_style_idx)

    def reverse_with_new_fillstyle(self, new_fill_idx):
        # 创建一个新的SWFStraightEdge实例，交换起始点和终点，并使用新的填充样式索引
...
class SWFCurvedEdge(SWFStraightEdge):
    def __init__(self, start, control, to, line_style_idx, fill_style_idx):
        super().__init__(start, to, line_style_idx, fill_style_idx)  # 使用 Python 3 的 super() 语法
        self.control = control

    def __repr__(self):
        # 提供类的字符串表示，方便调试
        return '<SWFCurvedEdge start={0}, control={1}, to={2}, line_style_idx={3}, fill_style_idx={4}>'.format(
            self.start, self.control, self.to, self.line_style_idx, self.fill_style_idx)

    def reverse_with_new_fillstyle(self, new_fill_idx):
        # 创建一个新的 SWFCurvedEdge 实例，交换起始点和终点，并使用新的填充样式索引
...
class SWFShape(_dumb_repr):
    ...
    def get_dependencies(self):
        """
        Collects all unique dependencies from fill styles and line styles.

        Returns:
            A set containing all unique dependencies.
        """
        s = set()
        # 收集 fill styles 的依赖项
    ...
    def parse(self,data,level=1):
        """
        Parses the shape data from the given bit stream.

        Args:
            data: The bit stream containing the shape data.
            level: The level of detail or nesting in the shape data.
    ...
    def export(self,handler = None):
        """
        Exports the shape data using a specified handler.

        Args:
            handler: An optional handler object that defines the export behavior.

        Returns:
            The handler object after export.
        """
    ...
    def read_shape_records(self, data, fill_bits, line_bits, level=1):
        """
        Reads shape records from the given data stream until an end record is encountered.

        Args:
            data: The data stream containing shape records.
            fill_bits: The number of bits used to encode fill styles.
            line_bits: The number of bits used to encode line styles.
            level: The level of detail in the shape data.
        ...
        shape_record = None
        record_id = 0
        while not isinstance(shape_record, SWFShapeRecordEnd):
        ...
        # 可以在这里处理或返回 self._records 列表
    def _create_edge_maps(self):
        """
        Creates edge maps for each shape group based on the shape records.
        """
