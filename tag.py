...
if __name__ == '__main__':
    ...
    # 收集所有Tag类
    tag_classes = [v for k, v in globals().items() if isinstance(v, type) and v.__name__.startswith('Tag') and hasattr(v, 'TYPE')]

    # 迭代收集的类，并创建实例
    for tag_class in tag_classes:
        tag_instance = TagFactory.create(tag_class.TYPE)
        if tag_instance is None:
            print(f"No instance created for {tag_class.__name__} with type {tag_class.TYPE}")
    # 其他处理...