...
def reason_unsupported(stream_or_tag):
    ...
    if header.soundFormat not in supportedCodecs:
        return 'codec {} ({}) not supported'.format(consts.AudioCodec.tostring(header.soundFormat),
                                                    header.soundFormat)
...
def write_sound_to_file(st, output):
    assert isinstance(st, tag.TagDefineSound), 'TagDefineSound expected'
    if st.soundFormat == consts.AudioCodec.MP3:
        swf_s = stream.SWFStream(st.soundData)
        seekSamples = swf_s.readSI16()
        output.write(swf_s.read())
